...
namespace Ostral.Core.Utilities
{
    ...
    {
        public static PaginatorResponseVM<IEnumerable<TDestination>> PaginationAsync<TSource, TDestination>(this IQueryable<TSource> queryable,
            int pageSize, int pageNumber, IMapper mapper)
            where TSource : class
            where TDestination : class
        {
            var count = queryable.Count();
            var pageResult = new PaginatorResponseVM<IEnumerable<TDestination>>
            {
